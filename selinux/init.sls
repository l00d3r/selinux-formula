...
{% for port in ports %}
{% set selinux_port_exists = salt['cmd.shell']('/usr/sbin/semanage port -l | grep -q {{ port }}') == '1' %}
{% set selinux_application_port_exists = salt['cmd.shell']('/usr/sbin/semanage port -l | grep {{ port }} | grep -q {{ application }}_port_t') == '1' %}
selinux_{{ application }}_{{ protocol }}_port_{{ port }}:
  cmd:
    - run
{% if selinux_port_exists is sameas true and selinux_application_port_exists is sameas false %}
