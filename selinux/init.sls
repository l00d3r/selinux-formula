...
{% for port in ports %}
{% set selinux_port_exists = salt['cmd.shell']('/usr/sbin/semanage port -l | grep -q ' ~ port ~ '; echo $?') %}
{% set selinux_application_port_exists = salt['cmd.shell']('/usr/sbin/semanage port -l | grep ' ~ port ~ ' | grep -q ' ~ application ~ '_port_t; echo $?') %}
selinux_{{ application }}_{{ protocol }}_port_{{ port }}:
  cmd:
    - run
{% if selinux_application_port_exists == '1' and selinux_port_exists == '0' %}
